...
export interface SettingsClientProps {
  /** Tab resolved from `?tab=` on the server. */
  initialTab: SettingsTab;
}

export function SettingsClient({ initialTab }: SettingsClientProps) {
  ...
  // A deep link may point at a tab this role cannot open.
  const activeTab =
    isPrivileged || !PRIVILEGED_TABS.includes(selectedTab)
      ? selectedTab
      : DEFAULT_SETTINGS_TAB;

  // The URL mirrors the tab so it stays shareable, but the swap is shallow —
  // a full navigation would re-fetch the page for a purely local change.
  const handleTabChange = useCallback(
    (value: string) => {
      ...
      const query = value === DEFAULT_SETTINGS_TAB ? "" : `?tab=${value}`;
  ...
  useEffect(() => {
    async function checkAzureStatus() {
      try {
        ...
        // Non-critical: the card simply renders as "not connected".
        console.error("[SettingsClient] checkAzureStatus:", error);
      } finally {
